...
class Scan(QWidget):
    ...
    def list_installed_packages(self, device):
        ...
        try:
            ...
            if packages:
                self.output_area.setText(f"Installed packages on {device}:\n" + "\n".join(packages))
            else:
                self.output_area.setText(f"No installed packages found on {device}.")
                QMessageBox.information(self, "No Packages", "No installed packages found.")
        except subprocess.CalledProcessError as e:
            self.output_area.setText(f"Error retrieving packages:\n{e.stderr}")
            QMessageBox.critical(self, "Error", "Failed to retrieve installed packages.")
    ...
    def extract_manifest(self, apk_path, output_dir, app_id):
        # app_id=Database.store_apk_in_db(apk_path, package_name)
        Database.log_scan(10,app_id)
